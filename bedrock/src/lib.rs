                    ...
                    if source.node == bedrock.our.node {
                        if let Some(path) = data.path {
                            ...
                            match bedrock.update_path(path) {
                                Err(_) => print_to_terminal(
                                    0,
                                    &format!("error updating path {}", str_path),
                                ),
                                Ok(_) => {
                                    print_to_terminal(0, &format!("updated path {}", str_path))
                                }
                            }
                        }
                    } else {
                        ...
                        if let Some(peers) = data.peers {
                            for peer in peers {
                                ...
                                match bedrock.delete_peer(&peer.id, &peer.path) {
                                    Err(_) => print_to_terminal(
                                        0,
                                        &format!("error deleting peer from {}", str_path),
                                    ),
                                    Ok(_) => print_to_terminal(
                                        0,
                                        &format!("deleted peer from {}", str_path),
                                    ),
                                }
                            }
                        }
                        ...
                        if let Some(rows) = data.rows {
                            for row in rows {
                                ...
                                match bedrock.delete_row(row.tbl_name(), id.clone(), row.path) {
                                    Err(_) => {
                                        print_to_terminal(0, &format!("error deleting row {}", id))
                                    }
